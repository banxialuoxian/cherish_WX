  ...
  doUpload: function() {
    ...
    wx.chooseImage({
      ...
      success: function(res) {
        ...
        var _imgUrl = new Array();
        var _cloudPath = new Array();
        var _temp = new Array();
        for (var j = 0; j < res.tempFilePaths.length; j++) {
          const filePath = res.tempFilePaths[j]
          _imgUrl.push(filePath)
          that.setData({
            filePath: filePath,
            imgUrls: _imgUrl
          })
          ...
          _cloudPath.push(cloudPath);
          that.setData({
            cloudPath: cloudPath,
            cloudPaths: _cloudPath
          })
          console.log(that.data.cloudPaths);
        }
        for (var j = 0; j < that.data.imgUrls.length; j++) {

          filePath: that.data.imgUrls[j]
          cloudPath: that.data.cloudPaths[j]
          wx.cloud.uploadFile({
            cloudPath: that.data.cloudPaths[j],
            filePath: that.data.imgUrls[j],
            success: res => {
              console.log('[上传文件] 成功：', res)
              _temp.push(res.fileID)
